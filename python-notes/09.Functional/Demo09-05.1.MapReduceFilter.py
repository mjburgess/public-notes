...
Why do we use map, filter, reduce?

MAP     To translate between different kinds of (plural) data, ages to heights, etc. 
FILTER  To irrelevant data: to filter children from a list. 
REDUCE  To summarize data: to get a total of prices. 

"""


# most programming problems can be phrased as transformations on data
# that is we have some initial input (perhaps a list of input()s)
...
print(list(range(0, 10)))  # list of ints
print(list(map(str, list(range(0, 10)))))  # list of strings

# ''.join([1, 2, 3]) # -- ERROR! Q. Why? A. ints cannot be concatenated

# so let's convert every element of our range list to strings
print(''.join(map(str, list(range(0, 10)))))

# or bools 
bool_list = list(map(bool, [True, False, None, 1, 0]))
print(bool_list)

# or formated strings
print(list(map(format, list(map(str, list(range(100, 500, 100)))))))

# FILTER:
# produces a new sequence from an old one using a test (or "predicate funciton")
...
print(list(filter(isAdult, input)))  # removes Sherlock Jnr!
...
print(reduce(operator.add, list(range(0, 10)), 0))
...
user_input = ['michael', 0, '', None, 'UK']
bool_list = list(map(bool, user_input))
...
''' OUTPUT (09.Functional/Demo09-05.1.MapReduceFilter.py):
Message: 100100

Message: [100, 200, 300][100, 200, 300]
...
['michael', 'UK']
[{'name': 'Sherlock', 'age': 27}, {'name': 'Watson', 'age': 35}]
30
1020
(10, 20, 10, 20)
10
