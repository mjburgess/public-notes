...
Why do we use types?
To determine the charactertistics and valid set of operations possible with a value. 
To layout the rules and patterns possible within our program (eg. eating requires food).

Why do we need our own types?
To write custom rules and custom operations for our data. 
"""
...
# the upper function is on the type, ie. it's in the type's namespace (a dictionary)

# If we can phrase programs as dictionary + functions which operate on dictionary
# then, why not group those operations into a special dictionary called a type? 
def make_person():
    return {
        # state
        'fname': 'Michael',
        ...
        # namespace
        '__namespace__': {'get_name': get_name},
...
me = make_person()
print(me['__namespace__']['get_name'](me))
...
''' OUTPUT (11.OO/Demo11-4.1.Types.py):
MichaelBurgess
<class '__main__.Car'>
<class '__main__.Car'>
100
